...
yum -y install git python3-pip >> /root/post-install.log
yum -y install --downloadonly gcc openssl-devel bzip2-devel libffi-devel binutils linux-api-headers libxml2-python >> /root/post-install.log
...
yum -y remote git python3-pip >> /root/post-install.log
